...
-- Final report showing constraints and created permissions
SELECT 
    'CONSTRAINTS' as report_section,
    'permissions_org_name_unique' as constraint_name,
    'UNIQUE(org_id, name)' as constraint_type,
    'Active' as status
UNION ALL
SELECT 
    'PERMISSIONS_CREATED' as report_section,
    o.name as org_name,
    p.name as permission_name,
    p.display_name as status
FROM public.permissions p
JOIN public.organizations o ON o.org_id = p.org_id
WHERE p.name IN ('manage_users', 'view_settings')
ORDER BY report_section, constraint_name, permission_name;